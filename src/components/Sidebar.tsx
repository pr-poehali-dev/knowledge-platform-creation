...
export function Sidebar({ currentPage, onNavigate, isAuthenticated }: SidebarProps) {
  const menuItems = [
    ...
    { id: 'knowledge' as Page, label: 'База знаний', icon: 'Library' },
    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-sidebar border-r border-sidebar-border flex flex-col">
      <div className="p-6 border-b border-sidebar-border">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-bold text-sidebar-foreground">EduPlatform</h1>
            <p className="text-xs text-sidebar-foreground/60">Обучающая платформа</p>
          </div>
        </div>
      </div>
        ...
        <div className="p-4 border-t border-sidebar-border">
          <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-sidebar-accent/50 cursor-pointer transition-colors">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-sidebar-foreground truncate">Иван Иванов</p>
              <p className="text-xs text-sidebar-foreground/60 truncate">student@edu.ru</p>
            </div>
          </div>
        </div>
      )}
